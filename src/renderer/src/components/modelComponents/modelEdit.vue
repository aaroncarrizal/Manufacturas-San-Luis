<template>
    <div class="container-lg">
        <p class="h1 text-center my-3">Editar modelo</p>
        <form @submit.prevent="editModel()">
            <div class="mb-3">
...
import { updateModel, getModel } from '../../../services/ModelService'
...
export default defineComponent({
    ...
    async beforeMount(){
        this.modelId = this.$route.params.id.toString()
        await this.loadModel(this.modelId)
    },
    methods: {
        async loadModel() {
            try{
                const res = await getModel(this.modelId)
                console.log(res)
                this.model = res.data
        ...
        async editModel() {
            try {
                const res = await updateModel(this.modelId,this.model)
                console.log(res)
                this.message = 'Modelo editado con éxito'
            } catch (error) {
                console.log(error)
                this.message = error
            }
